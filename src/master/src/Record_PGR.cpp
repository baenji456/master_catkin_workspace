...
LadybugError setGpsTimeSync(LadybugContext context, bool enable)
{
    ...
    timeSyncSettings.baudRate = 115200;
...
LadybugError init_camera() {
    ...
    switch (camInfo.deviceType) {
        ...
        case LADYBUG_DEVICE_LADYBUG5P:
        {
            ...
            m_jpegQualityPercentage = 80;
            m_isShutterAuto = true; //BEN changed to true for brighter pictures
            m_shutterTime = 0.5f;
            m_isGainAuto = false;
            m_gainAmount = 10;
            break;
...
LadybugError start_camera() {
    ...
    error = ladybugInitializeStreamForWriting(s_context, pszStreamNameToOpen, m_context, pszStreamNameOpened, false);
    if (error != LADYBUG_OK) {
    ...
    // HIER setGPSTimeSync aus Beispiel BEN
    // enable gps time sync
    const LadybugError TimeSyncError = setGpsTimeSync(m_context, true);
    if (TimeSyncError != LADYBUG_OK) {
