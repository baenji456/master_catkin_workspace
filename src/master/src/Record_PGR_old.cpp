...
double dBytesWritten;//BEN
unsigned long ulNumOfImgWritten;//BEN

ros::Publisher pub[LADYBUG_NUM_CAMERAS + 1];
ros::Publisher pub_info[LADYBUG_NUM_CAMERAS + 1];
...
/*
// BEN Enable GPS Time Synchronization
...
/** BEN
 * This function will publish Info for each camera
 */
//void publishInfo(ros::Time& timestamp, ros::NodeHandle nh, ros::Publisher& info_pub, long int& count, size_t camid) {

    // Create the message
//    sensor_msgs::CameraInfo msg;
//    GetMatricesFromFile(nh, msg, camid);
...
LadybugError start_camera() {
    ...
    // BEN Create the Stream context
    if ( s_context == NULL )
    {
        error = ladybugCreateStreamContext( &s_context );
        if (error != LADYBUG_OK) {
            throw std::runtime_error("Unable to create Stream context.");
        }
        ROS_INFO("Succesfully created Stream Context!");
    }

    const char* pszStreamNameToOpen;
    //char pszStreamBaseName = 'mypgr'
    pszStreamNameToOpen = "/home/benni/catkin_ws/0_Stream_Files/PGR/myPGR";
    //sprintf( pszStreamNameToOpen, "%s/%s",homedir, pszStreamBaseName );
    char* pszStreamNameOpened;

    // BEN Initialize Stream for writing
    error = ladybugInitializeStreamForWriting(s_context, pszStreamNameToOpen, m_context, pszStreamNameOpened, true);
    if (error != LADYBUG_OK) {
        ROS_ERROR("Error: Unable to Initialize Stream for writing (%s)", ladybugErrorToString(error));
        throw std::runtime_error("Unable to Initialize Stream for writing.");
    }
    ...
    {
        error = ladybugStopStream ( s_context );
    }
    //END BEN
...
LadybugError stop_camera() {
    /*
    // BEN
    const LadybugError StopRecError = ladybugStopStream(s_context);
    if (StopRecError != LADYBUG_OK) {
        ROS_ERROR("Error: Unable to stop Stream (%s)", ladybugErrorToString(StopRecError));
    }
    return StopRecError;
    // END BEN
    */
...
int main (int argc, char **argv)
{
    ...
    while (running_ && ros::ok()) {
        ...
        // BEN Write Image to Stream
        const LadybugError ImageWritingError = ladybugWriteImageToStream(s_context, &currentImage, &dBytesWritten, &ulNumOfImgWritten);
        if (ImageWritingError != LADYBUG_OK) {
            ROS_WARN("Failed write Image to disk. Error (%s). Trying to continue..", ladybugErrorToString(ImageWritingError) );
            continue;
        }
